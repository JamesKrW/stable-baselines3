...
class ParallelHybridActor(BasePolicy):
    ...
    def get_action_dist_params(self, obs: th.Tensor, ids: th.Tensor) -> Tuple[th.Tensor, th.Tensor, Dict[str, th.Tensor]]:
        ...
        log_stds = []
        idxs = []
        for id in range(self.action_dim_s):
            idx = th.where(ids == id)[0]
            feature_temp = features.index_select(0, idx)

            latent_pi = self.latent_pi_list[id](feature_temp)
        ...
        log_stds = th.cat(log_stds, dim = 0)
        mean_actions = th.cat(mean_actions, dim = 0)

        acc_itm = 0 
        inverse_index = th.zeros_like(ids)
        for idx in idxs:
            drang = th.arange(acc_itm, acc_itm + len(idx), 1, device=self.device)
            inverse_index.scatter_(0, idx, drang)
            acc_itm += len(idx)

        log_stds = log_stds.index_select(0, inverse_index)
        mean_actions = mean_actions.index_select(0, inverse_index)
...
class SEEDDiscreteHumanPolicy(BasePolicy):
    ...
    def _build(self, lr_schedule: Schedule) -> None:
        ...
        if self.share_features_extractor:
            critic_kwargs = self._update_features_extractor(self.critic_kwargs, features_extractor=self.actor.features_extractor)
            self.human_critic = self.make_critic(features_extractor=self.actor.features_extractor)
            # Do not optimize the shared features extractor with the critic loss
            # otherwise, there are gradient computation issues
            human_critic_parameters = [param for name, param in self.human_critic.named_parameters() if "features_extractor" not in name]
...
class SEEDSinglePolicy(BasePolicy):
    """
    Policy class (with both actor and critic) for SEED.
...
MlpSinglePolicy = SEEDSinglePolicy