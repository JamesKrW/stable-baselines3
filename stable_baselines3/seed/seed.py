...
from stable_baselines3.common.buffers import BaseBuffer, BalancedMixedReplayBuffer
...
class SEED(OffPolicyAlgorithm):
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            print(simulated_human_rewards)
            
            if self.rollback:
                if np.all(simulated_human_rewards < 0):
                    print("all feedback = 0, no action played")

                    new_obs, rewards, dones, infos = self._last_obs, [0] * env.num_envs, [0] * env.num_envs, [{"num_timesteps": 50, "num_ll_steps": 50, "num_hl_steps": 1}] * env.num_envs
                    for i in range(env.num_envs):
                        env.envs[i].total_steps += 1
                        env.envs[i].rewards.append(0)
                        env_i = env.envs[i].env
                        action = np.argmax(actions[i][:self.action_dim_s])
                        steps = env_i.skill.max_steps[env_i.skill.skill_indices[action]]
                        env_i.timestep += steps
                        infos[i] = {"num_timesteps": steps, "num_ll_steps": steps, "num_hl_steps": 1}
                        dones[i] = (env_i.timestep >= env_i.horizon)
                        if dones[i]:
                            ep_rew = sum(env.envs[i].rewards)
                            ep_len = len(env.envs[i].rewards)
                            ep_info = {"r": round(ep_rew, 6), "l": ep_len, "t": round(time.time() - env.envs[i].t_start, 6)}
                            infos[i] = {"num_timesteps": steps, "num_ll_steps": steps, "num_hl_steps": 1, 'episode':ep_info}
                            env.envs[i].reset()
                            # HACK: figure out why reset() doesn't reset timestep
                            env_i.timestep = 0
                        else:
                            infos[i] = {"num_timesteps": steps, "num_ll_steps": steps, "num_hl_steps": 1}
                            
                else:
                    states = [env.envs[i].env.sim.get_state() for i in range(env.num_envs)]
                    new_obs, rewards, dones, infos = env.step(actions)
                    for i in range(env.num_envs):
                        if simulated_human_rewards[i] < 0:
                            print("rolling back environment:", i)
                            env.envs[i].env.sim.set_state(states[i])
            else:
                new_obs, rewards, dones, infos = env.step(actions)
