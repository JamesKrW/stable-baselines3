...
from stable_baselines3.common.buffers import ReplayBuffer, DictReplayBuffer, SubGoalReplayBuffer, BalancedStageReplyBuffer
...
SelfMAPLE = TypeVar("SelfMAPLEStage", bound="MAPLEStage")


class MAPLEStage(OffPolicyAlgorithm):
    """
    MAPLE + Soft Actor-Critic + Subgoal Policy
    ...
    def __init__(
        ...
        replay_buffer_class: Optional[Type[ReplayBuffer]] = BalancedStageReplyBuffer,
    ...
    def _setup_model(self) -> None:
        ...
        self.replay_buffer = BalancedStageReplyBuffer(
            self.buffer_size,
            self.observation_space,
            self.action_space,
            device=self.device,
            n_envs=self.n_envs,
            n_stage=self.num_stages,
        )
    ...
    def collect_rollouts(
        ...
        num_collected_steps, num_collected_episodes, num_collected_transitions = 0, 0, 0
        ...
        while (train_freq.frequency * env.num_envs) > num_collected_transitions:
            if self.use_sde and self.sde_sample_freq > 0 and num_collected_steps % self.sde_sample_freq == 0:
                # Sample a new noise matrix
                self.actor.reset_noise(env.num_envs)

            stage = np.array(env.get_attr("cur_stage"))
            print("STAGES:",stage)

            actions, buffer_action = self._sample_action(learning_starts, action_noise, env.num_envs)
            ...
            num_collected_transitions += self._store_transition(replay_buffer, buffer_action, new_obs, rewards, dones, infos, stage) # only store data from current policy
    ...
    #     actions = np.zeros((len(stage), self.action_dim))
            
    #     # Predict actions from subgoal policies
    #     for i in range(self.learned_stages):
    #         if i in stage:
    #             action, _ = self.prev_subgoal_policies[i].predict(observation[stage == i], deterministic=True)
    #             actions[stage == i] = action
    #     # Predict actions from current policy
    #     if max(stage) >= self.learned_stages:
    #         action, _ = self.policy.predict(observation[stage >= self.learned_stages], deterministic=False)
    #         actions[stage >= self.learned_stages] = action
