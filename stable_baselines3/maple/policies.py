...
class HierarchicalActor(BasePolicy):
    """
    Actor network (policy) for MAPLE.
...
class CategoricalActor(BasePolicy):
    """
    Categorical Actor network (policy) for MAPLE.
    This is used as the task policy in MAPLE.
...
class ParallelHybridActor(BasePolicy):
    """
    Parallel Hybrid Actor network (policy) for MAPLE.
    This is used as primitive parameter subpolicies in MAPLE.
...
class MAPLEPolicy(BasePolicy):
    """
    Policy class (with both actor and critic) for MAPLE.
    ...
    def __init__(
        ...
        self.actor_kwargs.update(maple_kwargs)
